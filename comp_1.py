...
def component_detection_1(multiDiGraph: nx.MultiDiGraph, k, l):
    ...
    while edges_to_insert:
        ...
        if not edge_inserted:
            ...
            ### now let's start the component detection ###
              ...
              # if edge has been inserted
        pebbleCount = D.nodes[e[0]]['pebbles'] + D.nodes[e[1]]['pebbles']
        if pebbleCount <= l and pebbleCount > 0:  # if we have not more than l pebbles in u and v
            ...
            if pebbleCount == 0:  # if any w in reach(u,v) has at least one pebble return empty set
                ...
                reach_uv += [e[0], e[1]]  # Nicolas Vorschlag
                ...
                while len(queue) > 0:
                    ...
                    reach_w = list(
                        {node for listing in nx.dfs_successors(D, source=w).values() for node in listing}) + [w]
                    while u in reach_w: reach_w.remove(u)
                    while v in reach_w: reach_w.remove(v)
                    pebbleCount = 0
                    for __node in reach_w:
                        if __node not in e:  # check pebblecount in Reach(w) (without u and v)
                    ...
                    if pebbleCount == 0:
                        ...
                        '''for edge in D.edges():
                            if edge[0] in nodes_outside_reach and edge[1] in reach_w and edge[0] not in queue_memory:
                                queue.append(edge[0])
                                queue_memory.add(edge[0])'''
                component_list.append(list(set(V_prime)))
    ...
    print("componenten :",component_list)
