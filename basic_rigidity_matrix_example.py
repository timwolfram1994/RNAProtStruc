...
def rigidity_test(G):
    ...
    if rank_R == 3 * num_nodes - 1: # returns True if Graph is rigid and False if graph is flexible
        return True
    else:
        return False
    print(R)

print("The graph is rigid.") if rigidity_test(G) else print("The graph is flexible.")



def find_rigid_subgraph(G):

    '''iterate through graph but how?'''

    counter = 1
    while counter < G.number_of_nodes():
        nodeList = list(range(counter, counter+9, 1))
        print(nodeList)
        H = G.subgraph(nodeList)
        if rigidity_test(H):
            print("Rigid subgraph found.")
        counter += 1



'''return count, subgraph, marked pdb, marked, graph'''


find_rigid_subgraph(G)